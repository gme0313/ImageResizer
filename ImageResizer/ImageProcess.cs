...
namespace ImageResizer
{
    public class ImageProcess
    {
        ...
        /// <param name="destPath">產生圖片目的目錄路徑</param>
        /// <param name="scale">縮放比例</param>
        public void ResizeImagesAync(string sourcePath, string destPath, double scale)
        {
            ...
            {
                Task t1 = Task.Run( () => {
                     DoResize(filePath, destPath, scale);
                });
                tasks.Add(t1);
                //count++;
                //Console.WriteLine($"建立第 {count} 個Task");
            }

            Task.WaitAll(tasks.ToArray());
        }

        public void DoResize(string filePath, string destPath, double scale)
        {
            Image imgPhoto = Image.FromFile(filePath);
            string imgName = Path.GetFileNameWithoutExtension(filePath);
